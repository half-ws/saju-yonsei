...
if (typeof window !== 'undefined') {
  ...
  const loadAuthService = async () => {
    ...
    try {
      ...
      authService.onAuthStateChanged(async (user) => {
        window.__currentUserSync = user || null;
        if (user) {
  ...
  const updateAuthUI = (user) => {
    ...
    if (user) {
      const displayName = user.nickname || user.displayName || '사용자';
  ...
  window.__getCurrentUser = () => {
    return window.__currentUserSync || null;
  };

  // 관리자 확인
  window.__ADMIN_EMAIL = 'aksd3741@gmail.com';
  window.__isAdmin = () => {
    const user = window.__getCurrentUser();
    return user && user.email === window.__ADMIN_EMAIL;
  };
