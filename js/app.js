...
// 유틸리티
import { $, $id, delegate, debounce, setInnerHTML, visibility, classHelper, escapeHtml, safeInt } from './utils/dom.js';
...
class SajuApp {
  ...
  _renderFortune(result, gender) {
    ...
    if (gender) {
      try {
        ...
        if (daeunContainer) {
          daeunContainer.innerHTML = `<div class="fortune-section"><div class="section-title">대운 <span class="fortune-direction">${escapeHtml(termStr)}</span></div><div class="no-gender-msg">대운 계산 중 오류 발생</div></div>`;
        }
      }
    } else {
      // 성별 없음
      if (daeunContainer) {
        daeunContainer.innerHTML = `<div class="fortune-section"><div class="section-title">대운 <span class="fortune-direction">${escapeHtml(termStr)}</span></div><div class="no-gender-msg">성별을 선택하면 대운이 표시됩니다</div></div>`;
      }
      // 세운/월운은 성별 없이도 표시
      const curY = new Date().getFullYear();
      const birthY = result.input.year;
  ...
  _showInitError(error) {
    ...
    title.textContent = '⚠️ 앱 초기화 실패';

    const desc = document.createElement('p');
    desc.textContent = '죄송합니다. 앱을 시작하는 중 오류가 발생했습니다.';
  ...
  _handleUrlParams() {
    ...
    if (mode === 'gunghap') {
      ...
      // 입력값 검증 (URL 파라미터 보안)
      const ayVal = safeInt(ay, 0, 1900, 2100);
      const amVal = safeInt(am, 0, 1, 12);
      const adVal = safeInt(ad, 0, 1, 31);
      const ahVal = ah ? safeInt(ah, 0, 0, 23) : null;
      const amiVal = safeInt(ami, 0, 0, 59);
      const byVal = safeInt(by, 0, 1900, 2100);
      const bmVal = safeInt(bm, 0, 1, 12);
      const bdVal = safeInt(bd, 0, 1, 31);
      const bhVal = bh ? safeInt(bh, 0, 0, 23) : null;
      const bmiVal = safeInt(bmi, 0, 0, 59);

      // 유효한 값인지 검증
      if (ayVal < 1900 || amVal < 1 || adVal < 1 || byVal < 1900 || bmVal < 1 || bdVal < 1) {
      ...
      if (ay && am && ad && by && bm && bd) {
        // 본인 정보 설정
        $id('in-year').value = ayVal;
        $id('in-month').value = amVal;
        $id('in-day').value = adVal;
        if (ahVal !== null) $id('in-hour').value = ahVal;
        if (ami) $id('in-min').value = amiVal;
        if (ag) {
        ...
        setTimeout(() => {
          ...
          appState.emit('calculate', {
            year: ayVal, month: amVal, day: adVal,
            hour: ahVal, minute: amiVal,
            gender: ag === 'm' || ag === 'f' ? ag : 'm', hasTime: ahVal !== null
          });

          // 상대방 정보 설정 (궁합 렌더러가 초기화된 후)
          setTimeout(() => {
            $id('gh-year')?.setAttribute('value', byVal);
            $id('gh-month')?.setAttribute('value', bmVal);
            $id('gh-day')?.setAttribute('value', bdVal);
            const ghYear = $id('gh-year');
            const ghMonth = $id('gh-month');
            const ghDay = $id('gh-day');
            if (ghYear) ghYear.value = byVal;
            if (ghMonth) ghMonth.value = bmVal;
            if (ghDay) ghDay.value = bdVal;
            if (bhVal !== null) { const el = $id('gh-hour'); if (el) el.value = bhVal; }
            if (bmi) { const el = $id('gh-min'); if (el) el.value = bmiVal; }
            if (bg) {
...
if (typeof window !== 'undefined') {
  ...
  // 유명인 DB를 celebrity_db.json으로 내보내기 (콘솔에서 __exportCelebrityDb() 실행)
  window.__exportCelebrityDb = () => {
    const all = dbManager.getAll('celebrity');
    // contributor 메타데이터 제거하고 순수 데이터만 내보내기
    const clean = all.map(p => {
      const o = { name: p.name, year: p.year, month: p.month, day: p.day, hour: p.hour ?? '', min: p.min || 0, gender: p.gender || '', note: p.note || '' };
    ...
    a.href = url;
    a.download = 'celebrity_db.json';
    a.click();
    URL.revokeObjectURL(url);
    console.log(`[Export] ${clean.length} celebrities exported as celebrity_db.json`);
  };
  ...
  window.__showSajuGuide = () => {
    ...
    <div class="saju-section">
      ...
      <div class="saju-pillars">
        <div class="saju-pillar"><b>년주(年柱)</b><br>태어난 해</div>
        <div class="saju-pillar"><b>월주(月柱)</b><br>태어난 달</div>
        <div class="saju-pillar"><b>일주(日柱)</b><br>태어난 날</div>
        <div class="saju-pillar"><b>시주(時柱)</b><br>태어난 시간</div>
      </div>
    </div>
    ...
    <div class="saju-section">
      <h3>월지(月支) - 계절의 기운</h3>
      <p><b>월주의 지지</b>를 월지라 하며, 태어난 계절을 나타냅니다. 일간의 강약을 판단하는 가장 중요한 요소로, 사주의 전체적인 기운을 좌우합니다.</p>
      <div class="saju-ganzi">
        <div class="saju-gan">
          <b>봄 (목 왕)</b><br>
          인(寅)·묘(卯)·진(辰)월<br>
          <small>목(木)이 강한 계절</small>
        </div>
        <div class="saju-ji">
          <b>여름 (화 왕)</b><br>
          사(巳)·오(午)·미(未)월<br>
          <small>화(火)가 강한 계절</small>
        </div>
      </div>
      <div class="saju-ganzi" style="margin-top:8px">
        <div class="saju-gan">
          <b>가을 (금 왕)</b><br>
          신(申)·유(酉)·술(戌)월<br>
          <small>금(金)이 강한 계절</small>
        </div>
        <div class="saju-ji">
          <b>겨울 (수 왕)</b><br>
          해(亥)·자(子)·축(丑)월<br>
          <small>수(水)가 강한 계절</small>
        </div>
      </div>
    </div>
  ...
  const loadAuthService = async () => {
    ...
    try {
      ...
      authService.onAuthStateChanged(async (user) => {
        window.__setCurrentUser(user || null);
        if (user) {
          await window.__loadNickname(user);
          // 로그인 시 자동 운세 트리거
          setTimeout(() => { if (window.__triggerAutoFortune) window.__triggerAutoFortune(); }, 800);
        }
        updateAuthUI(user);
        // 피드백 수납 입력 UI 갱신
        if (window.__guestbookManager) {
          window.__guestbookManager.updateInputUI();
          window.__guestbookManager.render();
        }
      });

      return authService;
    } catch (e) {
      console.warn('인증 서비스 로드 실패:', e);
      return null;
    }
  };
  ...
  const updateAuthUI = (user) => {
    ...
    if (user) {
      const displayName = escapeHtml(user.nickname || user.displayName || '사용자');
      headerAuth.innerHTML = `
        <div class="user-info">
          ...
          <button class="btn-nickname-change" onclick="window.__showNicknameModal && window.__showNicknameModal()" title="프로필 설정" style="background:none;border:1px solid var(--border);border-radius:4px;padding:2px 6px;font-size:0.68rem;cursor:pointer;color:var(--text-dim)">프로필</button>
          <button class="btn-logout" onclick="window.__logout && window.__logout()">로그아웃</button>
        </div>
      `;
    } else {
      headerAuth.innerHTML = `
  ...
  // Google 로그인
  window.__loginWith = async (provider) => {
    ...
    if (result) {
      ...
      if (window.__showNotification) {
        window.__showNotification('로그인되었습니다.', 'success');
      }
      // 닉네임 미설정 시 설정 유도
      setTimeout(() => {
        const user = window.__getCurrentUser();
        if (user && !user.nickname) {
          window.__showNicknameModal();
        }
      }, 500);
    }
  };
  ...
  window.__logout = async () => {
    ...
    if (auth) {
      await auth.logout();
      if (window.__showNotification) {
        window.__showNotification('로그아웃되었습니다.', 'info');
      }
    }
  };

  // ═══ 자동 운세 ═══
  window.__triggerAutoFortune = () => {
    try {
      const user = window.__getCurrentUser();
      if (!user || !user.birthYear || !user.birthMonth || !user.birthDay) return;
      const result = SajuCalculator.calculate(
        user.birthYear,
        user.birthMonth,
        user.birthDay,
      ...
      if (result && window.__sajuApp && window.__sajuApp.renderers && window.__sajuApp.renderers.sidebar) {
        window.__sajuApp.renderers.sidebar._renderTodayFortune(result);
      }
    } catch (e) {
      console.warn('자동 운세 실패:', e);
    }
  };

  // 현재 사용자 가져오기 (보호된 저장소 - 콘솔에서 위조 방지)
  (() => {
    let _userStore = null;
    // setter: non-enumerable + non-configurable (콘솔 자동완성에 노출 안 됨)
    Object.defineProperty(window, '__setCurrentUser', {
      value: (user) => { _userStore = user || null; },
      writable: false, configurable: false, enumerable: false
    });
    Object.defineProperty(window, '__getCurrentUser', {
      value: () => _userStore,
      writable: false, configurable: false, enumerable: false
    });
    Object.defineProperty(window, '__currentUserSync', {
      get: () => _userStore,
      set: () => { console.warn('보안: __currentUserSync 직접 변경 차단됨'); },
      configurable: false
    });
  })();

  // 관리자 확인 (클로저로 보호 - 콘솔에서 변수 직접 변경 방지)
  (() => {
    const _adminKey = 'aksd3741@gmail.com';
    const _getAdmin = () => {
      const u = window.__getCurrentUser();
      return u && u.email === _adminKey && u.uid && u.provider === 'google.com';
    };
    Object.defineProperty(window, '__isAdmin', {
      get: () => _getAdmin,
      set: () => { console.warn('보안: __isAdmin 변경 시도 차단됨'); },
      configurable: false
    });
    // __ADMIN_EMAIL도 변경 불가
    Object.defineProperty(window, '__ADMIN_EMAIL', {
      get: () => _adminKey,
      set: () => { console.warn('보안: __ADMIN_EMAIL 변경 시도 차단됨'); },
      configurable: false
    });
  })();
  ...
  // 닉네임 유효성 검사: 한글, 영문, 숫자, 언더바만 허용
  window.__validateNickname = (nick) => {
    if (!nick || nick.length < 2 || nick.length > 16) return '닉네임은 2~16자여야 합니다.';
    if (!/^[가-힣a-zA-Z0-9_]+$/.test(nick)) return '한글, 영문, 숫자, 언더바(_)만 사용 가능합니다.';
    // 투명 문자, 공백 체크
    if (/\s/.test(nick)) return '공백은 사용할 수 없습니다.';
    return null;
  };

  // 닉네임 중복 확인 (Firestore)
  window.__checkNicknameUnique = async (nickname) => {
    try {
      ...
      // 본인의 문서인 경우 중복 아님
      const user = window.__getCurrentUser();
      if (user && snapshot.size === 1 && snapshot.docs[0].id === user.uid) return true;
      return false;
    } catch (e) {
  ...
  // 프로필 저장 (Firestore) — nickname + 생년월일시/성별
  window.__saveProfile = async (nickname, profileData) => {
    const user = window.__getCurrentUser();
    if (!user) return false;
    try {
      const saveData = { nickname, updatedAt: new Date() };
      if (profileData) {
        if (profileData.gender) saveData.gender = profileData.gender;
        if (profileData.birthYear) saveData.birthYear = profileData.birthYear;
        if (profileData.birthMonth) saveData.birthMonth = profileData.birthMonth;
        if (profileData.birthDay) saveData.birthDay = profileData.birthDay;
        saveData.birthHour = profileData.birthHour ?? null;
        saveData.birthMinute = profileData.birthMinute ?? null;
      }
      ...
      if (profileData) {
        user.gender = profileData.gender || null;
        user.birthYear = profileData.birthYear || null;
        user.birthMonth = profileData.birthMonth || null;
        user.birthDay = profileData.birthDay || null;
        user.birthHour = profileData.birthHour ?? null;
        user.birthMinute = profileData.birthMinute ?? null;
      }
      window.__setCurrentUser(user);
      localStorage.setItem('saju_user_profile', JSON.stringify({
        uid: user.uid, nickname,
        gender: user.gender, birthYear: user.birthYear, birthMonth: user.birthMonth,
        birthDay: user.birthDay, birthHour: user.birthHour, birthMinute: user.birthMinute
      }));
      updateAuthUI(user);
      return true;
    } catch (e) {
  ...
  // 하위 호환
  window.__saveNickname = async (nickname) => window.__saveProfile(nickname, null);

  // 프로필 로드 (Firestore → localStorage 폴백)
  window.__loadNickname = async (user) => {
    if (!user) return;
    // localStorage 먼저 (빠른 복원)
    try {
      const cached = JSON.parse(localStorage.getItem('saju_user_profile') || localStorage.getItem('saju_user_nickname') || '{}');
      if (cached.uid === user.uid) {
        if (cached.nickname) user.nickname = cached.nickname;
        if (cached.gender) user.gender = cached.gender;
        if (cached.birthYear) user.birthYear = cached.birthYear;
        if (cached.birthMonth) user.birthMonth = cached.birthMonth;
        if (cached.birthDay) user.birthDay = cached.birthDay;
        user.birthHour = cached.birthHour ?? null;
        user.birthMinute = cached.birthMinute ?? null;
        window.__setCurrentUser(user);
      }
    } catch (e) { /* 무시 */ }
    // Firestore에서 최신값
    try {
      if (typeof firebase !== 'undefined' && firebase.firestore) {
        ...
        if (doc.exists) {
          const d = doc.data();
          if (d.nickname) user.nickname = d.nickname;
          if (d.gender) user.gender = d.gender;
          if (d.birthYear) user.birthYear = d.birthYear;
          if (d.birthMonth) user.birthMonth = d.birthMonth;
          if (d.birthDay) user.birthDay = d.birthDay;
          user.birthHour = d.birthHour ?? null;
          user.birthMinute = d.birthMinute ?? null;
          window.__setCurrentUser(user);
          localStorage.setItem('saju_user_profile', JSON.stringify({
  ...
  // 닉네임 모달 표시
  window.__showNicknameModal = (callback) => {
    _nicknameCallback = callback || null;
    ...
    if (user) {
      const yr = byId('profile-birth-year');
      const mo = byId('profile-birth-month');
      const dy = byId('profile-birth-day');
      const hr = byId('profile-birth-hour');
      const mi = byId('profile-birth-minute');
      if (yr) yr.value = user.birthYear || '';
      if (mo) mo.value = user.birthMonth || '';
      if (dy) dy.value = user.birthDay || '';
      if (hr) hr.value = (user.birthHour != null && user.birthHour !== '') ? user.birthHour : '';
      if (mi) mi.value = (user.birthMinute != null && user.birthMinute !== '') ? user.birthMinute : '';
      // 성별 토글
      document.querySelectorAll('.profile-gender-btn').forEach(btn => {
        if (btn.dataset.gender === user.gender) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.profile-gender-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.profile-gender-btn').forEach(b => {
          if (b.dataset.gender === _selectedGender) {
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      const errMsg = window.__validateNickname(nick);
      if (errMsg) {
        if (error) { error.textContent = errMsg; error.style.display = 'block'; }
        return;
      }
      saveBtn.disabled = true;
      saveBtn.textContent = '확인 중...';
      const unique = await window.__checkNicknameUnique(nick);
      if (!unique) {
        if (error) { error.textContent = '이미 사용 중인 닉네임입니다.'; error.style.display = 'block'; }
      ...
      // 프로필 데이터 수집
      const byId = (id) => document.getElementById(id);
      const yr = parseInt(byId('profile-birth-year')?.value) || null;
      const mo = parseInt(byId('profile-birth-month')?.value) || null;
      const dy = parseInt(byId('profile-birth-day')?.value) || null;
      const hrVal = byId('profile-birth-hour')?.value;
      const miVal = byId('profile-birth-minute')?.value;
      const hr = (hrVal !== '' && hrVal != null) ? parseInt(hrVal) : null;
      const mi = (miVal !== '' && miVal != null) ? parseInt(miVal) : null;
      // 성별: 토글에서 선택된 값 또는 기존 값
      const user = window.__getCurrentUser();
      const gender = _selectedGender || user?.gender || null;
      ...
      const saved = await window.__saveProfile(nick, profileData);
      saveBtn.disabled = false;
      saveBtn.textContent = '저장';
      if (saved) {
        window.__hideNicknameModal();
        if (window.__showNotification) window.__showNotification('프로필이 저장되었습니다.', 'success');
        if (_nicknameCallback) { _nicknameCallback(); _nicknameCallback = null; }
        // 자동 운세 트리거
        if (window.__triggerAutoFortune) window.__triggerAutoFortune();
      } else {
        if (error) { error.textContent = '저장에 실패했습니다. 다시 시도해주세요.'; error.style.display = 'block'; }
      }
    });
  ...
  // 로그인 필요 확인 (사주 저장, 인물 추가, 방명록 작성 시 사용)
  window.__requireLogin = async (callback) => {
    const auth = await loadAuthService();
