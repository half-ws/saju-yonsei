...
if (typeof window !== 'undefined') {
    ...
    if (user) {
      ...
      headerAuth.innerHTML = `
        <div class="user-info">
          ...
          <button class="btn-nickname-change" onclick="window.__showNicknameModal && window.__showNicknameModal()" title="닉네임 변경" style="background:none;border:1px solid var(--border);border-radius:4px;padding:2px 6px;font-size:0.68rem;cursor:pointer;color:var(--text-dim)">닉네임</button>
  ...
  // 닉네임 저장 (Firestore)
  window.__saveNickname = async (nickname) => {
    const user = window.__getCurrentUser();
    ...
    try {
      ...
      localStorage.setItem('saju_user_nickname', JSON.stringify({ uid: user.uid, nickname }));
      updateAuthUI(user);
      return true;
    } catch (e) {
  ...
  // 닉네임 로드 (Firestore → localStorage 폴백)
  window.__loadNickname = async (user) => {
    if (!user) return;
    // localStorage 먼저 (빠른 복원)
    try {
      const cached = JSON.parse(localStorage.getItem('saju_user_nickname') || '{}');
      if (cached.uid === user.uid && cached.nickname) {
    ...
    try {
      if (typeof firebase !== 'undefined' && firebase.firestore) {
        ...
        if (doc.exists && doc.data().nickname) {
          ...
          window.__setCurrentUser(user);
          localStorage.setItem('saju_user_nickname', JSON.stringify({ uid: user.uid, nickname: user.nickname }));
        }
      }
    } catch (e) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      const saved = await window.__saveNickname(nick);
      saveBtn.disabled = false;
      saveBtn.textContent = '저장';
      if (saved) {
        window.__hideNicknameModal();
        if (window.__showNotification) window.__showNotification(`닉네임이 '${nick}'으로 설정되었습니다.`, 'success');
