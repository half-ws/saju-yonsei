  ...
  analyzeCompatibilityFull(rA, rB, htA, htB) {
    ...
    (() => {
      ...
      if (commonBujok.length) {
        s -= 8 * commonBujok.length;
        notes.push(`[오행] 공통부족(${commonBujok.join(',')}) 보완 불가: -${8 * commonBujok.length}`);
      }
      ...
      if (common.length) {
        if (hasHapAny) {
          ...
          s -= 10;
          notes.push(`[오행] 발달오행 겹침(${common})+충 존재: -10`);
        }
      }
...
export class GunghapRenderer {
  ...
  _setupEventListeners() {
    ...
    document.getElementById('gh-year')?.addEventListener('input', function() { smartLimit(this, 2100); });
    document.getElementById('gh-month')?.addEventListener('input', function() { smartLimit(this, 12); });
    document.getElementById('gh-day')?.addEventListener('input', function() { smartLimit(this, 31); });
    document.getElementById('gh-hour')?.addEventListener('input', function() { smartLimit(this, 23); });
    document.getElementById('gh-min')?.addEventListener('input', function() { smartLimit(this, 59); });
  ...
  _calculate() {
    ...
    try {
      ...
      if (resultsEl) {
        resultsEl.innerHTML = `<div class="error-msg">계산 중 오류가 발생했습니다: ${error.message}</div>`;
  ...
  findBestMatch() {
    ...
    container.innerHTML = `
      <div class="bm-loading">
        <div id="bm-progress">궁합 분석 중... 0%</div>
        <div class="bm-prog-bar">
          <div id="bm-prog-fill" class="bm-prog-fill"></div>
        </div>
      </div>
    `;
    container.style.display = 'block';
    ...
    finder.findBestMatch(
      (pct) => {
        const pEl = document.getElementById('bm-progress');
        const pFill = document.getElementById('bm-prog-fill');
        if (pEl) pEl.textContent = `궁합 분석 중... ${pct}%`;
  ...
  _renderBestMatch(data) {
    ...
    const { bestList, worstList, dist, dMap, elapsed } = data;
    const r = this.person1Result;
    const ohengColors = { 목: '#2d8a4e', 화: '#c0392b', 토: '#b8860b', 금: '#7f8c8d', 수: '#2874a6' };
    ...
    // 점수 범위 배너
    const best = bestList[0], worst = worstList[0];
    html += `<div class="bm-banner">
      <div class="bm-banner-range">
        <span style="color:#c0392b">${worst.norm}점</span>
        <span style="color:var(--text-dim);font-size:1rem">~</span>
        <span style="color:#2d8a4e">${best.norm}점</span>
      </div>
    </div>`;

    // 다양성 체크 함수 (년주 다름 + 일주/월주 4개 중 2개 이상 다름)
    const checkDiversity = (existing, candidate) => {
      // 년주 겹치면 불가
      if (existing.yKey === candidate.yKey) return false;
      // 일간, 일지, 월간, 월지 중 겹치는 개수 (2개까지 허용)
      let overlap = 0;
      if (existing.dsi === candidate.dsi) overlap++;
      if (existing.dbi === candidate.dbi) overlap++;
      if (existing.msi === candidate.msi) overlap++;
      if (existing.mbi === candidate.mbi) overlap++;
      return overlap <= 2; // 2개까지 겹쳐도 OK (2개 이상 달라야 함)
    };

    // 최고/최악 조합 선택 (1등은 무조건, 나머지는 기준 + 다양성)
    const selectTop3 = (list, threshold, isHigher) => {
      if (list.length === 0) return [];
      const result = [list[0]]; // 1등은 무조건
      for (let i = 1; i < list.length && result.length < 3; i++) {
        const m = list[i];
        const passThreshold = isHigher ? m.norm >= threshold : m.norm <= threshold;
        if (!passThreshold) continue;
        // 다양성 체크 (result에 있는 모든 항목과 비교)
        const isDiverse = result.every(x => checkDiversity(x, m));
        if (isDiverse) result.push(m);
      }
      return result;
    };

    const filteredBest = selectTop3(bestList, 90, true);
    if (filteredBest.length > 0) {
      html += `<div class="section-title" style="margin:20px 0 12px">🏆 최고의 궁합</div>`;
      html += `<div class="bm-match-grid">`;
      for (let i = 0; i < filteredBest.length; i++) {
        html += matchCard(filteredBest[i], i + 1, '🏆', '최고의 궁합', '#2d8a4e');
      }
      html += `</div>`;
    }

    // 최악 조합 (1등은 무조건 표시, 나머지 2개는 20점 이하 + 다양성)
    const filteredWorst = selectTop3(worstList, 20, false);
    if (filteredWorst.length > 0) {
      html += `<div class="section-title" style="margin:20px 0 12px">💀 최악의 궁합</div>`;
      html += `<div class="bm-match-grid">`;
      for (let i = 0; i < filteredWorst.length; i++) {
        html += matchCard(filteredWorst[i], i + 1, '💀', '최악의 궁합', '#c0392b');
      }
      html += `</div>`;
    }
      ...
      for (; diIdx < end; diIdx++) {
        ...
        for (let yi = 0; yi < 60; yi++) {
          ...
          for (let mn = 1; mn <= 12; mn++) {
            ...
            // 다양성 체크: 년주 안겹침 + 일간/일지/월간/월지 중 2개 이상 달라야 함
            const isDiverse = (list, c) => {
              for (const x of list) {
                // 년주 겹치면 불가
                if (x.yKey === c.yKey) return false;
                // 일간, 일지, 월간, 월지 중 겹치는 개수 계산 (2개까지만 허용)
                let overlap = 0;
                if (x.dsi === c.dsi) overlap++;
                if (x.dbi === c.dbi) overlap++;
                if (x.msi === c.msi) overlap++;
                if (x.mbi === c.mbi) overlap++;
                if (overlap > 2) return false; // 3개 이상 겹치면 불가
              }
              return true;
            };
