...
export class GunghapRenderer {
  ...
  _calculate() {
    ...
    try {
      ...
      // 이스터에그: 특별한 두 사람
      const d1 = `${p1Year}-${p1Month}-${p1Day}`, d2 = `${year}-${month}-${day}`;
      const eggPair = (d1 === '2003-3-29' && d2 === '2002-2-9') || (d1 === '2002-2-9' && d2 === '2003-3-29');
      ...
      // 이스터에그 점수 오버라이드
      if (eggPair) {
        compatResult.scores.total = 100000000;
        compatResult.summary = '우주가 증명한 운명적 인연! 1억점의 완벽한 궁합입니다.';
      }
  ...
  _renderResultsFull(c) {
    ...
    const isEasterEgg = sc >= 100000000;
    const scColor = isEasterEgg ? '#e6b422' : sc >= 70 ? '#2d8a4e' : sc >= 45 ? '#b8860b' : '#c0392b';
    ...
    // 점수 헤더
    if (isEasterEgg) {
      html += `<div class="gh-score-center gh-easter-egg">
        <div class="gh-score-num" style="color:#e6b422;font-size:2.5rem;text-shadow:0 0 20px rgba(230,180,34,0.5)">1억<span style="font-size:1.2rem;font-weight:400;color:var(--text-dim)">/100</span></div>
        <div class="gh-score-label" style="font-size:1.1rem;color:#e6b422">✨ 우주가 증명한 운명적 인연 ✨</div>
      </div>`;
    } else {
      html += `<div class="gh-score-center">
  ...
  _renderBestMatch(data) {
    ...
    // 다양성 체크 함수 (일주 다름 필수 + 년주 다름)
    const checkDiversity = (existing, candidate) => {
      // 일주 겹치면 불가 (일간+일지 조합이 같으면 안됨)
      if (existing.dsi === candidate.dsi && existing.dbi === candidate.dbi) return false;
      // 년주 겹치면 불가
