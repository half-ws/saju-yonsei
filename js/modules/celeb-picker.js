...
export class CelebPickerRenderer {
  ...
  _updateLoginUI() {
    ...
    if (this.currentDbType === 'personal') {
      ...
      if (this.snsUser) {
        ...
        const safeUserName = escapeHtml(this.snsUser.name || '');
      ...
      if (this.snsUser) {
        ...
        // 기여 횟수 및 등급 계산
        const userUid = this.snsUser.uid || this.snsUser.email;
  ...
  _showAddModal() {
    ...
    // XSS 방지: 사용자 정보 이스케이프
    const contributorName = escapeHtml(this.snsUser.name || '익명');
    const contributorProvider = escapeHtml(this.snsUser.provider || 'unknown');
