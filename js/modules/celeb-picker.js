...
export class CelebPickerRenderer {
  ...
  _renderList() {
    ...
    // 다중 정렬 적용
    list.sort((a, b) => {
      // 사용자 정렬이 없으면: 사용자 추가 인물(contributor 있는)을 최신순으로 상단 배치
      if (this.sortStack.length === 0) {
        const aHas = a.contributor && a.contributor.addedAt ? 1 : 0;
        const bHas = b.contributor && b.contributor.addedAt ? 1 : 0;
        if (aHas !== bHas) return bHas - aHas;
        if (aHas && bHas) return new Date(b.contributor.addedAt) - new Date(a.contributor.addedAt);
      }
    ...
    pageList.forEach((person, idx) => {
      ...
      if (c && c.ds >= 0) {
        ...
        try {
          ...
          // 대운 (gender 문자열 전달: 'm' 또는 'f')
          const daeunData = DaeunCalculator.calculate(result, person.gender || 'f');
      ...
      if (person.source) {
        ...
        sourceHtml = `<span class="db-src" title="${escapeHtml(person.source)}">출처: ${srcText}</span>`;
      }

      // 기여자 표시
      let contributorHtml = '';
      if (person.contributor) {
        const cName = escapeHtml(person.contributor.nickname || person.contributor.name || '익명');
        contributorHtml = `<span class="db-contributor">기여자: ${cName}</span>`;
      }
      ...
      html += `<div class="db-person-card db-card-clickable" data-index="${safeGlobalIdx}">
        ...
        ${sourceHtml}
        ${window.__isAdmin && window.__isAdmin() ? `<button class="btn-db-delete-mini" data-index="${safeGlobalIdx}" title="삭제">×</button>` : ''}
      </div>`;
      } catch (e) {
    ...
    container.querySelectorAll('.btn-db-delete-mini').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        if (!window.__isAdmin || !window.__isAdmin()) return;
        const idx = parseInt(btn.dataset.index);
        if (confirm(`'${list[idx].name}'을(를) 삭제하시겠습니까?`)) {
          dbManager.removePerson(list[idx]._i, this.currentDbType);
          this._renderList();
  ...
  _selectPerson(person) {
    try {
      ...
      // result 객체에 인물 정보 직접 첨부 (렌더러에서 읽음)
      result.personName = person.name || '';
      result.personNote = person.note || '';

      // 실시간 인기 사주 트래킹
      if (window.__trackCelebView && person.name) {
        const pillars = result.saju ?
          [result.saju.yearPillar, result.saju.monthPillar, result.saju.dayPillar, result.saju.hourPillar]
            .map(p => p ? (p.stemHanja || '') + (p.branchHanja || '') : '').join(' ') : '';
        window.__trackCelebView(person.name, pillars);
      }
      ...
      if (isFromGunghap) {
        ...
        // 계산기 입력 필드에 생년월일시 대입
        const yearEl = document.getElementById('in-year');
        const monthEl = document.getElementById('in-month');
        const dayEl = document.getElementById('in-day');
        const hourEl = document.getElementById('in-hour');
        const minEl = document.getElementById('in-min');
        if (yearEl) yearEl.value = person.year || '';
        if (monthEl) monthEl.value = person.month ? String(person.month).padStart(2, '0') : '';
        if (dayEl) dayEl.value = person.day ? String(person.day).padStart(2, '0') : '';
        if (hourEl) hourEl.value = hasTime ? String(hour).padStart(2, '0') : '';
        if (minEl) minEl.value = hasTime ? String(minute).padStart(2, '0') : '';

        // 성별 버튼 갱신
        const gm = document.getElementById('gender-m');
        const gf = document.getElementById('gender-f');
        if (gm) gm.classList.toggle('active', person.gender === 'm');
        if (gf) gf.classList.toggle('active', person.gender === 'f');
      }
  ...
  _showAddModal() {
    ...
    // 닉네임 미설정 시 닉네임 설정 유도
    if (!user.nickname) {
      if (window.__showNicknameModal) {
        window.__showNicknameModal(() => this._showAddModal());
      } else {
    ...
    const contributorName = escapeHtml(user.nickname || user.displayName || '익명');
    const contributorProvider = escapeHtml(user.provider || 'google');
    ...
    // 이벤트 - 닫기 시 확인
    const confirmClose = () => {
      if (confirm('정말 작성을 취소하시겠습니까?')) modal.remove();
    };
    modal.querySelector('.modal-close').addEventListener('click', confirmClose);
    modal.querySelector('.btn-cancel').addEventListener('click', confirmClose);
    modal.querySelector('.btn-confirm').addEventListener('click', () => {
      ...
      const result = dbManager.addPerson({
        ...
        contributor: {
          ...
          nickname: user.nickname || contributorName,
    ...
    // 배경 클릭으로 닫기 방지 (X 또는 취소만으로 닫기 가능)
  }
