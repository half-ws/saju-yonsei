...
export class BTIRenderer {
  ...
  render(result, hasTime, ohengData, yongsinData, daeunData, gender) {
    ...
    // 사주명식 카드
    html += '<div class="pillars-section" style="margin-bottom:20px;"><h3 class="section-title">사주명식</h3>';
    html += '<div class="pillars-grid" style="grid-template-columns:repeat(4,1fr)">';
    ...
    if (!hasTime) {
      html += `
        <div class="pillar-card empty-pillar">
          ...
          <div class="char-sub" style="color:#ccc">시간 미상</div>
          <div class="pillar-divider"></div>
          <div class="char-block">
            <span class="char-kr" style="color:#ddd">?</span>
          </div>
    ...
    const pillarPositions = hasTime ? ['hour', 'day', 'month', 'year'] : ['day', 'month', 'year'];
    const pillarLabels = { hour: '시주', day: '일주', month: '월주', year: '년주' };
    ...
    for (const pos of pillarPositions) {
      ...
      const branchOheng = JIJI_OHENG[branchIdx];
      const stemClass = getOhengClass(stemOheng);
      const branchClass = getOhengClass(branchOheng);
      const dayClass = pos === 'day' ? ' day-pillar' : '';
      ...
      html += `
        <div class="pillar-card${dayClass}">
          <div class="pillar-label">${pillarLabels[pos]}</div>
          <div class="ten-god-stem">${result.tgStem[pos]}</div>
          <div class="char-block">
            <span class="char-kr ${stemClass}">${CHEONGAN[stemIdx]}</span>
            <span class="char-cn ${stemClass}">${CHEONGAN_HANJA[stemIdx]}</span>
          </div>
          <div class="char-sub">${stemSign}${stemOheng}</div>
          <div class="pillar-divider"></div>
          <div class="char-block">
            <span class="char-kr ${branchClass}">${JIJI[branchIdx]}</span>
            <span class="char-cn ${branchClass}">${JIJI_HANJA[branchIdx]}</span>
          </div>
          <div class="char-sub">${branchSign}${branchOheng}</div>
          <div class="ten-god-branch">${result.tgBranch[pos]}</div>
          <div class="twelve-stage">${tsDisplay}</div>
        </div>
      `;
    }
    ...
    if (ilgan) {
      ...
      // 고유 개성 (있는 경우)
      if (ilgan.unique) {
        html += `<div class="bti-unique-box"><span class="bti-unique-label">💫 ${ilgan.name}만의 개성</span><p class="bti-unique-text">${ilgan.unique}</p></div>`;
      }
      ...
      // 고서 해석 (있는 경우)
      if (ilgan.classic) {
        html += `<div class="bti-classic-box"><span class="bti-classic-label">📜 고서 해석</span><p class="bti-classic-text">${ilgan.classic}</p></div>`;
      }
    ...
    if (ilju) {
      ...
      if (ilju.strength || ilju.weakness) {
        html += `<div class="bti-unique-box">`;
        html += `<span class="bti-unique-label">강점과 약점</span>`;
        if (ilju.strength) html += `<p class="bti-unique-text"><strong>강점:</strong> ${ilju.strength}</p>`;
        if (ilju.weakness) html += `<p class="bti-unique-text"><strong>약점:</strong> ${ilju.weakness}</p>`;
        html += `</div>`;
      }
      if (ilju.career) {
        html += `<div class="bti-classic-box">`;
        html += `<span class="bti-classic-label">적합 직업</span>`;
        html += `<p class="bti-classic-text">${ilju.career}</p>`;
        html += `</div>`;
      }
      if (ilju.love) {
        html += `<div class="bti-unique-box">`;
        html += `<span class="bti-unique-label">연애/결혼</span>`;
        html += `<p class="bti-unique-text">${ilju.love}</p>`;
        html += `</div>`;
      }
      if (ilju.tips) {
        html += `<div class="bti-unique-box">`;
        html += `<span class="bti-unique-label">개운 TIP</span>`;
        html += `<p class="bti-unique-text">${ilju.tips}</p>`;
        html += `</div>`;
      }
      html += `</div>`;
    }
    ...
    if (wolju) {
      ...
      // 월주 해석 설명 (사회적 재능 관점)
      html += `<div class="bti-unique-box"><span class="bti-unique-label">💼 사회적 재능과 의지</span>`;
      html += `<p class="bti-unique-text">월주는 의식적으로 발현되는 사회적 재능과 직업적 역량을 나타냅니다. 일주가 무의식적 성격이라면, 월주는 사회에서 의지적으로 발휘하는 능력입니다.</p></div>`;
      ...
      if (wolju.strength || wolju.weakness) {
        html += `<div class="bti-unique-box">`;
        html += `<span class="bti-unique-label">사회적 강점과 약점</span>`;
        if (wolju.strength) html += `<p class="bti-unique-text"><strong>강점:</strong> ${wolju.strength}</p>`;
        if (wolju.weakness) html += `<p class="bti-unique-text"><strong>약점:</strong> ${wolju.weakness}</p>`;
        html += `</div>`;
      }
      if (wolju.career) {
        html += `<div class="bti-classic-box">`;
        html += `<span class="bti-classic-label">적합 직업 (월주 기반)</span>`;
        html += `<p class="bti-classic-text">${wolju.career}</p>`;
        html += `</div>`;
      }
      html += `</div>`;
    }
    ...
    // 오행 해석 카드
    html += `<div class="bti-card"><div class="bti-section-title">오행 해석</div>`;

    // 강한/약한 오행 해석 (이미 정의된 strongOhList, weakOhList 사용)
    if (strongOhList.length > 0) {
      html += `<div class="bti-unique-box"><span class="bti-unique-label">🔼 강한 오행</span>`;
      for (const [oh, pct] of strongOhList) {
        const ohInfo = OHENG_INTERPRETATION[oh];
        if (ohInfo) {
          html += `<div style="margin:12px 0;padding:12px;background:rgba(0,0,0,0.03);border-radius:10px;">`;
          html += `<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">`;
          html += `<span style="font-size:1.3rem;">${ohInfo.emoji}</span>`;
          html += `<span style="font-weight:700;color:${UI.COLORS.OHENG[oh]}">${ohInfo.name} (${Math.round(pct)}%)</span>`;
          html += `</div>`;
          html += `<p style="color:var(--text-secondary);font-size:0.9rem;line-height:1.6;margin:0;">${ohInfo.strong}</p>`;
          html += `</div>`;
        }
      }
      html += `</div>`;
    ...
    if (weakOhList.length > 0) {
      html += `<div class="bti-classic-box"><span class="bti-classic-label">🔽 약한 오행</span>`;
      for (const [oh, pct] of weakOhList) {
        ...
        if (ohInfo) {
          ...
          html += `<span style="font-weight:700;color:${UI.COLORS.OHENG[oh]}">${ohInfo.name} (${Math.round(pct)}%)</span>`;
          html += `</div>`;
          html += `<p style="color:var(--text-secondary);font-size:0.9rem;line-height:1.6;margin:0;">${ohInfo.weak}</p>`;
          html += `<p style="color:var(--accent);font-size:0.85rem;line-height:1.5;margin:8px 0 0 0;"><strong>💡 보완법:</strong> ${ohInfo.balance}</p>`;
          html += `</div>`;
        }
      }
      html += `</div>`;
    ...
    // 오행 전체 분포 요약
    html += `<div style="margin-top:16px;padding:12px;background:linear-gradient(135deg,rgba(74,144,226,0.08),rgba(155,89,182,0.08));border-radius:10px;">`;
    html += `<div style="font-weight:600;margin-bottom:8px;color:var(--text);">📊 오행 분포 특성</div>`;
    ...
    if (presentOh === 5 && !hasWeakOh) {
      html += `<p style="color:var(--text-secondary);font-size:0.9rem;line-height:1.6;margin:0;">오행이 모두 고르게 갖춰져 있습니다. 균형 잡힌 기운으로 어느 한쪽으로 치우치지 않는 안정적인 성격을 가집니다.</p>`;
    } else if (presentOh === 5 && hasWeakOh) {
      html += `<p style="color:var(--text-secondary);font-size:0.9rem;line-height:1.6;margin:0;">오행이 모두 갖춰져 있으나 <strong>${weakOhList.map(([k]) => k).join(', ')}</strong> 오행이 약합니다. 약한 오행의 기운을 보충하면 더욱 균형 잡힌 삶을 살 수 있습니다.</p>`;
    } else {
      const missingOh = ['목', '화', '토', '금', '수'].filter(oh => !ohPctSorted.find(([o, v]) => o === oh && v > 0));
      if (missingOh.length > 0) {
        html += `<p style="color:var(--text-secondary);font-size:0.9rem;line-height:1.6;margin:0;">사주에 <strong>${missingOh.join(', ')}</strong> 오행이 없습니다. 없는 오행의 기운을 보충하면 더욱 균형 잡힌 삶을 살 수 있습니다.</p>`;
      }
    }
    html += `</div>`;
    ...
    if (topGroupPct > 0) {
      ...
      if (mainSipsungInfo) {
        ...
        html += `<div class="bti-card-name">${mainSipsung} (${topGroupName} ${topGroupPct}%)</div>`;
        ...
        if (mainSipsungInfo.career1) {
          html += `<div class="bti-unique-box"><span class="bti-unique-label">💼 적합 직업 (1순위)</span><p class="bti-unique-text">${mainSipsungInfo.career1}</p></div>`;
        }

        // 고서 해석
        if (mainSipsungInfo.classic) {
          html += `<div class="bti-classic-box"><span class="bti-classic-label">📜 고서 해석</span><p class="bti-classic-text">${mainSipsungInfo.classic}</p></div>`;
        }
      }
      ...
      if (tsGroupSorted.length > 1 && tsGroupSorted[1][1] >= 15) {
        ...
        if (ts2Info) {
          ...
          html += `<div class="bti-card-name">${ts2Sipsung} (${ts2Name} ${ts2Pct}%)</div>`;
          ...
          // 적합 직업
          if (ts2Info.career1) {
            html += `<div class="bti-unique-box"><span class="bti-unique-label">💼 적합 직업 (1순위)</span><p class="bti-unique-text">${ts2Info.career1}</p></div>`;
          }

          // 고서 해석
          if (ts2Info.classic) {
            html += `<div class="bti-classic-box"><span class="bti-classic-label">📜 고서 해석</span><p class="bti-classic-text">${ts2Info.classic}</p></div>`;
          }
        }
      }
      html += `</div>`;
    ...
    // 2026년 병오년 기본 정보
    html += `<div class="bti-unique-box">`;
    html += `<span class="bti-unique-label">🔥 2026년 병오년(丙午年)</span>`;
    ...
    // 병오년 특성
    html += `<div class="bti-classic-box">`;
    html += `<span class="bti-classic-label">📘 병오년 전체 운세 특성</span>`;
    ...
    const ILGAN_2026_INTERPRETATION = {
      ...
      임: {
        ...
        flow: {
          상반기: '시장조사, 수익모델 설계, 셀링·포트폴리오 정비. 생산자 포지션으로 이동하세요.',
          하반기: '효율·수익을 올리는 학습(마케팅/브랜딩/세일즈). 수익·현금흐름을 잡는 힘이 강해집니다.',
          주의시기: '2026년 하반기 말~2027년 상반기: 분위기에 휩쓸려 돈을 쉽게 내놓는 위험이 있습니다.'
        },
      ...
      계: {
        ...
        flow: {
          '2026년': '2027년 도약을 위한 준비 기간입니다. 조직·사람을 통해 성과→보상으로 연결하세요.',
          '2027년': '가치·성과·역할을 먼저 잡으면 돈이 결과로 따라옵니다. 흐름이 더 선명해집니다.',
          주의: '번아웃, 돈 거래, 관계 선별에 주의하세요. 돈 욕망은 말로 드러내지 말고 내부 동력으로만 쓰세요.'
        },
    ...
    if (ilgan2026) {
      html += `<div style="margin-top:16px;padding:16px;background:linear-gradient(135deg,rgba(204,51,51,0.08),rgba(255,165,0,0.08));border-radius:12px;border:1px solid rgba(204,51,51,0.2);">`;

      // 헤더
      html += `<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">`;
      html += `<span style="font-size:1.8rem;">${ilgan2026.emoji}</span>`;
      html += `<div><span style="font-weight:700;font-size:1.2rem;color:var(--text);">${ilgan2026.name}의 2026년</span>`;
      html += `<span style="display:block;font-size:0.85rem;color:#cc3333;font-weight:600;">${ilgan2026.relation}</span></div>`;
      html += `</div>`;
      ...
      // 요약
      html += `<p style="font-weight:600;color:var(--text);font-size:1.05rem;margin:0 0 12px 0;padding:10px;background:rgba(204,51,51,0.1);border-radius:8px;text-align:center;">${ilgan2026.summary}</p>`;
      ...
      if (ilgan2026.caution && ilgan2026.caution.length > 0) {
        html += `<div style="margin-bottom:10px;"><span style="font-weight:600;color:#c0392b;font-size:0.9rem;">⚠️ 주의할 점</span>`;
        html += `<ul style="margin:6px 0 0 20px;color:var(--text-secondary);font-size:0.85rem;line-height:1.7;">`;
      ...
      // 조언
      if (ilgan2026.advice) {
        html += `<div style="padding:12px;background:linear-gradient(135deg,rgba(46,204,113,0.1),rgba(39,174,96,0.1));border-radius:8px;border:1px solid rgba(46,204,113,0.3);">`;
